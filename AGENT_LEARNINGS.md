...
**What went wrong:** Claude Code reads/writes its keychain item by forking `/usr/bin/security`, so the item's ACL partition list is exactly `("apple-tool:")`. Clauge's DMG build read it in-process (`security_framework`), which securityd logs as `ACL partition mismatch: client cdhash:… ACL ("apple-tool:")` — a password dialog. "Always Allow" was genuinely recorded (`adding XARA partition 'cdhash:…'`) and then wiped when Claude Code rewrote the item: granted 13:25:09, gone by 15:42:02 BDT the same day. All three prompts in the 35-hour log window fired mid-session against a Clauge process that had been up 27 days — not at login. The "silent after Always Allow" claim in `keychain.rs` / `keychain_cache.rs` (and in the wizard, README and PRIVACY.md) was a documented assumption with no test and no log evidence behind it. Worse, the first-pass investigators converged on Developer-ID signing, the MAS build, and a manual Keychain Access edit as fixes; fresh-context sceptics refuted all three with primary evidence (the app binary was unchanged for 61 days yet the grant still died in 2h17m; the MAS flavor falls back to the identical in-process read; a manual grant lives in the same list that gets wiped). Then the review of the fix itself caught a second-order hole nobody had designed for: `serde_json` invalid-type errors embed the offending value, so a wrong-shaped blob could have carried the token into `log::warn!` and the webview — on all four read paths, three of them years old.

**Lesson:** when you read ANOTHER app's keychain item, access belongs to that app's write path, not to you — read it the way its owner does, and never write "click once, silent forever" into a comment, a wizard or a privacy page without evidence across the owner's next rewrite.

**Prevention:** (a) the DMG flavor reads through `/usr/bin/security`, the in-process reader is cfg-gated to MAS, and a mutation-verified wiring-guard test fails if the DMG path regresses; (b) any keychain change is verified on a real surface by streaming securityd's `integrity` category for `ACL partition mismatch`, never by unit tests alone; (c) run the cheapest decisive probe BEFORE writing code — here one `security find-generic-password … -w` piped into a key-names-only `jq` (exit 0, 0.025 s, zero mismatch lines) proved the fix would hold; (d) adversarially verify root-cause reports AND the fix — investigator convergence was wrong three times in one afternoon, and the reviewers' 23-shape leak battery found 5 leaking shapes the implementer's own "garbage input" test never exercised; (e) never let a parser's error text near a secret — sanitize to category + position; (f) a guard assertion built on `contains("feature = \"mas\"")` also matches `not(feature = "mas")` — assert exact equality and mutation-verify EVERY assertion, not just the first; (g) log forensics: use the absolute `/usr/bin/log` (zsh's builtin `log` shadows it and fails silently into a pipe), count with `--style ndjson` + jq, and remember your own `security` calls pollute same-day correlations.

**Hotfix:** v1.3.9 — `read_via_security_cli()` + `parse_creds_blob()` + `KeychainError::SecurityCli` in `src-tauri/src/keychain.rs`; false "Always Allow" claims corrected in `keychain.rs`, `keychain_cache.rs`, `ipc.rs` comments, the first-run wizard, README and PRIVACY.md. **Still open** (AGENTS.md landmine #51 "known holes"): the MAS flavor keeps prompting; no timeout on the `security` child while `get_or_load` holds its Mutex; the expired-token re-read failure is logged at `debug` (below the shipped filter); every `KeychainError` renders as "not installed" in the Connections tab; `cargo clippy --features mas` has pre-existing errors and no CI job builds that feature.

**Cross-references:** AGENTS.md landmine #51; auto-memory `reference_keychain_apple_tool_partition_wipe.md`; global rulebook entry same date; upstream anthropics/claude-code #77697, #89985, #87348, #94491; prior art steipete/CodexBar `docs/keychain-prompts.md`.
