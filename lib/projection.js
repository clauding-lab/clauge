...
/**
 * The recent-burst rate (last 15-60 min) only represents the trajectory for
 * SHORT windows. On a multi-day window, extrapolating the last hour's pace
 * across the huge time-to-reset spuriously trips `will_hit` for almost any
 * active user — a brief burst implies "100% in N hours" you'd never sustain.
 * Windows longer than this forecast from the sustained window-average instead.
 */
export const RECENT_RATE_MAX_WINDOW_MS = 18000000; // 5h — only the fiveHour window uses the recent rate
...
export function projectWindow({ pct, resetsAt, windowMs, nowMs, history }) {
  ...
  // Short windows (≤5h) use the responsive recent-burst rate; long windows
  // (7-day) forecast from the sustained window-average so a momentary burst
  // doesn't extrapolate to a spurious "100% before reset" (see
  // RECENT_RATE_MAX_WINDOW_MS).
  const recentRate =
    windowMs <= RECENT_RATE_MAX_WINDOW_MS
      ? computeRecentRate({ pct, resetsAtMs, nowMs, history })
      : null;
