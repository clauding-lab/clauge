...
export class AlertState {
   ...
   * rename) so a crash mid-write can never leave a torn file. The read-then-
   * write is NOT lost-update-safe on its own; safety comes from the
   * single-writer invariant (landmine #40 — the sidecar is the sole writer and
   * its alert loop marks sequentially). Re-reading keeps markFired idempotent
   * and union-preserving across separate sequential calls.
