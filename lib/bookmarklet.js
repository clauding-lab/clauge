...
(async () => {
  ...
  try {
    ...
    // Probe candidate balance endpoints. claude.ai is the consumer chat;
    // platform.claude.com is the API developer console (where the
    // prepaid credit balance lives). Same auth cookies on both domains.
    let balance = null;
    const candidates = [
      // claude.ai (extra_usage cap-side)
      '/api/organizations/' + org.uuid + '/billing',
      '/api/organizations/' + org.uuid + '/billing/balance',
      '/api/organizations/' + org.uuid + '/credits',
      // platform.claude.com (API console prepaid balance)
      'https://platform.claude.com/api/organizations/' + org.uuid + '/billing',
      'https://platform.claude.com/api/organizations/' + org.uuid + '/billing/balance',
      'https://platform.claude.com/api/organizations/' + org.uuid + '/credits',
      'https://platform.claude.com/api/organizations/' + org.uuid + '/account',
    ];
