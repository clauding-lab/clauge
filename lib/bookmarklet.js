...
(async () => {
  ...
  try {
    ...
    // Try to also capture billing/credit balance — multiple candidate paths,
    // first 200 wins. claude.ai's exact endpoint isn't documented; this
    // probes the likely ones and ignores 404s silently.
    let balance = null;
    const candidates = [
      '/api/organizations/' + org.uuid + '/billing',
      '/api/organizations/' + org.uuid + '/billing/balance',
      '/api/organizations/' + org.uuid + '/credits',
      '/api/organizations/' + org.uuid + '/account/balance',
    ];
