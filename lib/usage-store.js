...
export function normalizeOverageSpendLimit(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const limit = Number(raw.monthly_credit_limit);
  const used = Number(raw.used_credits);
  const hasLimit = Number.isFinite(limit);
  const hasUsed = Number.isFinite(used);
  if (!hasLimit && !hasUsed) return null;

  // Claude.ai returns these in DOLLARS (e.g. 10.0 = $10), not cents — that's
  // confirmed by CodexBar's same parser. If a future schema flips to cents,
  // it'll need a divisor here.
  const usedDollars = hasUsed ? used : null;
  const limitDollars = hasLimit ? limit : null;
  let pct = null;
