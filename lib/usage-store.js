 ...
 * payload. We probe several candidate field shapes since the underlying
 * claude.ai endpoint isn't formally documented.
 */
export function normalizeBalance(rawBalance, org) {
  // Try, in order: explicit balance object, then any field on the org
  // returned by /api/organizations that looks billing-shaped.
  const sources = [];
  ...
  for (const s of sources) {
    const balanceCents =
      ...
      s.balance_cents ??
      (s.current_balance != null && typeof s.current_balance === 'number' && s.current_balance > 1000
        ? s.current_balance
        : null);
    const balanceDollars =
      ...
      (s.current_balance != null && typeof s.current_balance === 'number' && s.current_balance <= 1000
        ? s.current_balance
        : null) ??
      (balanceCents != null ? balanceCents / 100 : null);

    if (balanceDollars == null) continue;

    const reload = s.auto_reload ?? s.autoReload ?? null;
    const enabled =
      reload != null
        ? reload === true || reload?.enabled === true
        : s.auto_reload_enabled === true;
    const reloadAmount =
