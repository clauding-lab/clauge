...
import { WINDOW_KEYS, SAME_WINDOW_TOLERANCE_MS } from './projection.js';
...
// The window allowlist (WINDOW_KEYS) and the same-window tolerance are imported
// from lib/projection.js — the single source of truth. Adding a window there
// automatically extends sampling here; the legacy raw-codename duplicates
// normalizeUsage also emits (sevenDayOmelette, sevenDayCowork) and the
// non-window fields (extraUsage, unknownSevenDayKeys) are never in WINDOW_KEYS,
...
// kept rising (so it was NOT a reset).
const DRIFT_ZONE_MIN_MS = SAME_WINDOW_TOLERANCE_MS; // 5 min
...
export class UsageHistory {
  ...
  /**
   * Read the history file ONCE and bucket every line into all WINDOW_KEYS in a
   * single pass. Returns a map { [key]: samples[] } with one (possibly empty)
   * oldest-first array per window key — the shape buildProjection consumes.
   * Equivalent to calling samplesFor for every key, but reads + parses the file
   * once instead of once per key (the /api/projection handler is webview-polled,
   * so the 6× redundant full reads are worth collapsing).
   *
   * @returns {Promise<Record<string, Array<{ at: string, pct: number | null, resetsAt: string | null }>>>}
   */
  async samplesByWindow() {
    const out = {};
    for (const key of WINDOW_KEYS) out[key] = [];
    let raw;
    try {
      raw = await readFile(this.filePath, 'utf8');
    } catch {
    ...
    for (const line of raw.split('\n')) {
      ...
      for (const key of WINDOW_KEYS) {
        const win = obj.w?.[key];
        if (!win || typeof win !== 'object') continue;
        out[key].push({ at: obj.at, pct: win.pct ?? null, resetsAt: win.resetsAt ?? null });
      }
    }
    return out;
  }
