...
export async function run(parsed, deps = {}) {
  ...
  // The three data sources are independent — fetch them concurrently so the
  // worst case is max(fetch 250ms, git 300ms, transcript read), not the sum.
  const text = stdinText !== undefined ? stdinText : await readStdinBounded();
