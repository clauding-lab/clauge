...
function buildLine1(payload, branch, paint, homeDir) {
  ...
  if (typeof model === 'string' && model !== '') segments.push(paint.magenta(model));

  const dir = payload?.workspace?.current_dir ?? payload?.cwd;
  if (typeof dir === 'string' && dir !== '') {
    segments.push(paint.blue(abbreviateHome(dir, homeDir)));
  }
  ...
  if (typeof branch === 'string' && branch !== '') segments.push(paint.magenta(branch));
  return segments;
}

// ── Line 2: quota gauges — Session and Weekly progress lines from the
//    /v1 snapshot, each colored independently on its own value.
function buildLine2(snapshot, paint, nowMs) {
  ...
  for (const label of ['Session', 'Weekly']) {
    ...
    if (!line || !Number.isFinite(line.used)) continue;
...
function buildLine3(snapshot, payload, compactions, paint) {
  ...
  if (spend && typeof spend.value === 'string') {
    const m = /^(\$[\d,.]+)\s+(.+)$/.exec(spend.value);
    segments.push(m ? `${paint.yellow(m[1])} ${paint.dim(m[2])}` : paint.dim(spend.value));
  }

  const roi = textLine('ROI (30d)') ?? textLine('ROI');
  if (roi && typeof roi.value === 'string') {
    const m = /^(-?[\d,.]+)x\s+(.+)$/.exec(roi.value);
    segments.push(
      m ? `ROI ${paint.yellow(`${m[1]}×`)} ${paint.dim(m[2])}` : `ROI ${paint.dim(roi.value)}`,
    );
  }
