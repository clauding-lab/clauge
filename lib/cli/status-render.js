...
//
// Color grammar (owner-locked 2026-07-18):
//   magenta  model + git branch          yellow  money ($ spend, ROI) ONLY
//   blue     working path                orange  gauge warning ≥75% ONLY
//   green/red lines added/removed        red     gauge ≥90%, compactions 2+
//   cyan     session runtime             dim     separators, labels' context
// Gauges (Session / Weekly / Context Used) color INDEPENDENTLY: green <75,
// orange ≥75, red ≥90. Compactions has its own scale: green 0 / orange 1 /
// red 2+. Orange is 256-color (\x1b[38;5;208m) with plain yellow as the
// 8/16-color fallback — never collapse orange into money-yellow on capable
// terminals. Everything must still read with colors stripped: the numbers
// carry the meaning, color is emphasis.
//
...
// render as additional line-2 gauges, wire order, capped at 2 for line-width
// protection.
...
function compactionPaint(count, paint) {
  if (count >= 2) return paint.red;
  if (count >= 1) return paint.orange;
  return paint.green;
}
...
// <rel>)`, colored independently via gaugePaint, resets suffix omitted when
// resets_at is absent. Returns null when `used` isn't a finite number.
function buildGaugeSegment(label, line, paint, nowMs) {
  if (!Number.isFinite(line.used)) return null;
  const colorize = gaugePaint(line.used, paint);
  let segment = `${label} ${colorize(bar(line.used))} ${colorize(`${line.used}%`)}`;
  const resets = line.resets_at != null ? formatResets(line.resets_at, nowMs) : null;
  if (resets) segment += ` ${paint.dim(`(resets ${resets})`)}`;
  return segment;
}

// ── Line 2: quota gauges — Session and Weekly progress lines from the
//    /v1 snapshot, each colored independently on its own value. Rev 5 adds
//    up to SCOPED_GAUGES_MAX scoped-limit gauges (e.g. `Weekly (Fable)`)
//    after the two hero gauges, in wire order.
function buildLine2(snapshot, paint, nowMs) {
  ...
  for (const line of snapshot.lines ?? []) {
    ...
    const segment = buildGaugeSegment(sanitize(match[1]), line, paint, nowMs);
    if (!segment) continue;
...
// ── Line 3: value + hygiene. Spend/ROI parse the frozen /v1 string formats
//    ("$N this window", "N.Nx vs API" — lib/api-v1.js, landmine #47); on any
//    drift they fall back to the raw value rather than crash. The widget
//    prefers the monthly 'ROI (30d)' line (owner decision 2026-07-18) and
//    falls back to the frozen 7d 'ROI' line.
function buildLine3(snapshot, payload, compactions, paint) {
  const segments = [];
  const textLine = (label) =>
    snapshot?.lines?.find((l) => l?.type === 'text' && l?.label === label) ?? null;

  const spend = textLine('Spend');
  if (spend && typeof spend.value === 'string') {
    const value = sanitize(spend.value);
    const m = /^(\$[\d,.]+)\s+(.+)$/.exec(value);
    segments.push(m ? `${paint.yellow(m[1])} ${paint.dim(m[2])}` : paint.dim(value));
  }

  const roi = textLine('ROI (30d)') ?? textLine('ROI');
  if (roi && typeof roi.value === 'string') {
    const value = sanitize(roi.value);
    const m = /^(-?[\d,.]+)x\s+(.+)$/.exec(value);
    segments.push(
      m ? `ROI ${paint.yellow(`${m[1]}×`)} ${paint.dim(m[2])}` : `ROI ${paint.dim(value)}`,
    );
  }

  const ctx = payload?.context_window?.used_percentage;
  if (Number.isFinite(ctx)) {
    segments.push(`Context Used ${gaugePaint(ctx, paint)(`${ctx}%`)}`);
  }

  if (Number.isFinite(compactions)) {
    segments.push(`Compactions ${compactionPaint(compactions, paint)(String(compactions))}`);
  }
  return segments;
}
...
export function renderStatus(inputs, opts = {}) {
  ...
  const line3 = buildLine3(snapshot, payload, compactions, paint);
