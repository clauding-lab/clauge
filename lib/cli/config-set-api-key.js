...
//
// v0.9.3 scope: --provider anthropic-admin only. As of v1.0.0 the Rust
// IPC layer (`src-tauri/src/ipc.rs::{set,get,clear}_anthropic_api_key`)
// reads and writes the same Keychain item — the dashboard's Settings
// UI and this CLI are now interchangeable entry points for the user's
...
//
// Keychain account convention: account is the literal string "default"
// to match the Rust IPC reader (`ipc.rs::ANTHROPIC_API_KEY_KEYCHAIN_SERVICE`
// neighborhood). Locked by AGENTS.md landmine #14 — the JS CLI and Rust
// reader must agree on BOTH service AND account, or the user's CLI paste
// lands in a different Keychain entry than the dashboard reads. Don't
...
export async function run(parsed, ioOpts = {}) {
  ...
  // Literal "default" — matches the Rust IPC reader. See AGENTS.md
  // landmine #14 and the header comment of this file.
