...
**Things that do NOT fix the prompt — each refuted with log evidence, do not re-propose them as "the keychain fix":** Developer-ID signing + notarization (changes the entry from `cdhash:` to `teamid:`, but it lives in the same list that gets wiped — still worth doing for Gatekeeper and update-safety); adding Clauge by hand in Keychain Access or via `set-generic-password-partition-list` (lifetime: hours); "Allow all applications" (insecure); the MAS build (the App Sandbox cannot use the CLI path; it still reads in-process and still prompts).
