...
Any new frontend JS the packaged sidecar must SERVE — under `public/` (dashboard) or `popover/lib/` (popover, via the gitignored `public/popover/` mirror) — must be added to BOTH SEA asset manifests: the `scripts/sea-config.json` assets map AND the `scripts/sea-bootstrap.cjs` `ASSETS` array, beside the sibling already there (e.g. `public/popover/lib/copy.js`). Creating the file + wiring its `<script>` tag is NOT enough: in dev (`node server.js`, Hono `serveStatic`) it works, but the packaged DMG/MAS SEA build only serves files enumerated in those two manifests → a missing entry is a SILENT 404 → the global the file defines is `undefined` → the consumer throws ONLY in the shipped build. **`npm run check` does NOT catch this (no SEA stage); `npm run test:sea` is the gate — run it after adding any served file.** (v1.2.0 Task 7 added `popover/lib/swr.js` and missed this; caught by review, fixed `8c962fe`. Generalizes the SEA-manifest-mirror landmine to *any* served JS, not only the originally-named assets.)
