...
Writing source or docs through JSON-carrying editor tools turns a literal ` `-style sequence in the payload into the actual control BYTE, silently corrupting the file to binary (bit an Edit call on 2026-07-18 and a Write call + a subagent Edit on 2026-07-19 — three hits in two days). Safe constructions when a control-char range is needed: JS — build the RegExp via `String.fromCharCode` (see `lib/usage-store.js`); Swift — `CharacterSet.controlCharacters`; prose/docs — spell ranges as `U+0000-U+001F`. After ANY write that involves escape-bearing content, byte-scan the file: `python3 -c "b=open('<file>','rb').read(); print([hex(c) for c in b if (c<0x20 and c not in (9,10,13)) or c==0x7f])"` must print `[]`. Repairs go through python byte-replace, never another Edit.
