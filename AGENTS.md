...
### 37. `SNAPSHOT_SCHEMA_VERSION` is a CROSS-REPO contract — a bump requires a clauge-ios release FIRST

`lib/snapshot.js`'s `SNAPSHOT_SCHEMA_VERSION` (currently 1) is enforced by the iOS companion since clauge-ios 1.0.1: `AnalyticsSnapshot.supportedSchemaVersion` gates every snapshot read, and a NEWER version is never best-effort parsed (iOS shows "Update this app"; missing/older/corrupt shows "Update Clauge on your Mac"). The two apps ship on independent release trains (Mac: DMG auto-update lands within hours + MAS; iOS: App Store review takes days), so a Mac-side bump that ships first strands every iPhone user on a dead Analytics tab until Apple approves the matching iOS build. ORDER: (1) clauge-ios PR bumps `supportedSchemaVersion` + handles the new shape, (2) that iOS build is approved and LIVE, (3) only then does the Mac bump ship. When bumping, also update `test/snapshot.test.js`'s `schemaVersion === 1` assertion and the `icloud_publish.rs` stamp-test fixture (it hardcodes `"schemaVersion": 1`).

### 38. ESM repo — dual-mode browser/node test helpers are IIFE + `window` global + `vm.runInThisContext`, NOT CommonJS

`package.json` is `"type": "module"`, so a `.js` file is parsed as ESM (no `require`) and a classic browser `<script>` throws on `module.exports`/`export`. A "dual-mode CommonJS" helper (the pattern a plan may specify for pure browser-logic shared with `node:test`) does NOT work here. The working pattern (v1.2.0 Tasks 7-9, files `popover/lib/swr.js` + `public/swr.js`): the helper is a pure browser **IIFE** that sets a `window.Claude*` global (mirroring `popover/heatmap.js` / `popover/lib/copy.js`); the browser loads it as a classic `defer` script BEFORE its consumer; the `node:test` loads the REAL shipped source file via `vm.runInThisContext(src, …)` with a fake `{ window }`. Use `runInThisContext`, NOT `runInNewContext`, so the helper's returned objects share the test realm's prototypes — otherwise `assert.deepStrictEqual` rejects structurally-identical cross-realm objects. Do NOT add `module.exports` / `require` / `export` to these files.

### 39. A NEW served frontend JS file must be in BOTH SEA manifests — `npm run check` won't catch a miss, only `npm run test:sea` does

Any new frontend JS the packaged sidecar must SERVE — under `public/` (dashboard) or `popover/lib/` (popover, via the gitignored `public/popover/` mirror) — must be added to BOTH SEA asset manifests: the `scripts/sea-config.json` assets map AND the `scripts/sea-bootstrap.cjs` `ASSETS` array, beside the sibling already there (e.g. `public/popover/lib/copy.js`). Creating the file + wiring its `<script>` tag is NOT enough: in dev (`node server.js`, Hono `serveStatic`) it works, but the packaged DMG/MAS SEA build only serves files enumerated in those two manifests → a missing entry is a SILENT 404 → the global the file defines is `undefined` → the consumer throws ONLY in the shipped build. **`npm run check` does NOT catch this (no SEA stage); `npm run test:sea` is the gate — run it after adding any served file.** (v1.2.0 Task 7 added `popover/lib/swr.js` and missed this; caught by review, fixed `8c962fe`. Generalizes the SEA-manifest-mirror landmine to *any* served JS, not only the originally-named assets.) Note: a new *server-side* `lib/` module (NOT served to the browser) needs NO manifest entry — esbuild pulls it into `dist/server.bundle.mjs` via the import graph automatically. The two manifests are ONLY for browser-served assets.

### 40. The sidecar must NOT persist state in the Tauri `settings.json` — the Rust iCloud-publish loop clobbers it every ~300s

`settings.json` (`configPaths.settingsFile()`, the tauri-plugin-store file) has TWO would-be writers: the Node sidecar (`lib/settings-writer.js`, provider toggles) and the Rust parent. Since v1.1.0 the Rust iCloud-publish loop (`src-tauri/src/icloud_publish.rs`) calls `store.save()` at least every 300s (stamping `icloud_snapshot_seq` / upload-status keys), and tauri-plugin-store rewrites the WHOLE file from its in-memory map → **any key the sidecar wrote after the store loaded is silently erased within minutes** on every iCloud-signed-in Mac (the flagship config). So: **persist sidecar-owned state in a SIDECAR-owned file** (e.g. `~/.clauge/config.json` beside `usage.json`, atomic tmp+rename — the pattern `lib/config-store.js` uses for the editable subscription cost), which has exactly one writer. The in-code "implausible race" note on `settings-writer.js` PREDATES the v1.1.0 recurring writer and is stale. (Surfaced building active-guardrail sub-project A; `lib/config-store.js` deliberately avoided `settings.json` for this reason.) **Known latent exposure:** the existing provider-toggle writes via `settings-writer.js` carry this same clobber risk — a real follow-up bug, not yet fixed (toggles are rarely touched, so low-frequency).

### 41. `SessionStore.loadAllSummaries()` must cap parse concurrency — never an unbounded `Promise.all` over every session file

`lib/session-store.js::loadAllSummaries()` parses session files in bounded batches of `SUMMARY_LOAD_CONCURRENCY` (8). The pre-v1.3.0 version fanned an unbounded `Promise.all(files.map(loadSummary))` over EVERY file; on a cold in-memory cache with a large `~/.claude` (1.1 GB / 2647 files) it parsed the whole transcript corpus at once and OOM-killed the sidecar at ~4 GB within ~50s (warm steady state is ~57 MB — the cache holds only small summaries, so the blow-up is purely transient parse memory). The bug was pre-existing and dormant — caches warm incrementally in normal use, so no single run ever faced a full-corpus cold parse — but sub-project B's Rust alert poller hits this path (`/api/alerts/pending` → `buildLiveProjection` → `loadAllSummaries`) every 30s from launch, turning it into an every-launch crash loop for heavy users. Do NOT "simplify" the batching back to a bare `Promise.all`. `test/session-store.test.js` pins the cap (mutation-verified: it fails against the unbounded version). The warm path (stat-only cache hits) is unaffected by the cap, so there is no perf reason to remove it; ~18 endpoints share this loader. See AGENT_LEARNINGS 2026-06-13.

### 42. `buildSnapshot` re-publishes `forecast`/`forecastHistory` VERBATIM from `buildLiveProjection` — never recompute, never bump the schema

C1.5 (paired enrichment) makes the Mac's iCloud snapshot carry two OPTIONAL top-level keys the iOS app reads to sharpen its on-device forecast: `forecastHistory` (`{fiveHour, sevenDay}` recent-burn samples) + `forecast` (`{weekOverWeek, roiPace}`). The forecast values are pulled VERBATIM from `buildLiveProjection(nowMs)` (`server.js`, extended to return `{record, projection, history}`) — `projection.windows.sevenDay.weekOverWeek` and the top-level `projection.roiPace` — and are NEVER recomputed inside `snapshot.js`, so the snapshot cannot drift from `/api/projection` (the no-drift property; gated by a verbatim-threading unit test in `test/snapshot.test.js` + the `/api/snapshot` wiring test in `test/server-snapshot-forecast.test.js`). `/api/snapshot` captures ONE `nowMs` and threads `now`/`history`/the two forecast values so the 60-min `forecastHistory` filter agrees with the projection's clock. RULES: `forecastHistory` is hero-windows-only, samples within `RECENT_SPAN_MS` (60 min — reuse the constant from `lib/projection.js`), `pct` 0–100 UNCHANGED (do NOT rescale — `usage-history.js` already stores engine units). **`SNAPSHOT_SCHEMA_VERSION` STAYS 1** — both keys are optional (old iOS ignores unknown keys); a bump hard-fails the live iOS `decodeWire` and blanks the Analytics tab on every phone until it updates. Alerts stay OUT of the snapshot (landmine #37). The Rust writer's `seq`/`writerId` stamping is field-agnostic, so the new keys are transparent to it; existing snapshot/iCloud fixtures stay valid (new `buildSnapshot` params default to `null`/no-op). (Built C1.5 desktop, PR #52.)

### 43. Dashboard flex-row layouts must be deterministic by construction — WebKit/WKWebView sizes flex children by text max-content, so Chromium/Playwright won't reproduce wrap bugs
...
### 46. Spawned-server tests: ports must be unique across ALL test files, and CLAUGE_HOME does NOT isolate UsageStore

`node --test` runs test FILES in parallel, so two files spawning `node server.js` on the same port collide only in the full `npm run check` — standalone runs stay green, which is exactly how it slips through. Before picking a port, census BOTH declaration styles: `grep -rn "const PORT" test/` AND `grep -rn "PORT:" test/` (the 2026-07-18 /v1 suite grabbed 3493/3494, silently owned by `cors-allowlist`/`path-trim` via `const PORT` — the mid-suite symptom was `fetch failed` → ECONNREFUSED, with the ingest answered by the WRONG file's server). Map as of 2026-07-18: 3493–3541 taken, 3550/3551 = server-v1. Second half: `UsageStore` resolves `~/.clauge/usage.json` via raw `os.homedir()` — `CLAUGE_HOME` does not redirect it, so a "cold state" spawned server reads the developer's real usage data. Isolate spawned servers with `HOME` + `USERPROFILE` env overrides (redirects every homedir-based store uniformly). Copy the harness in `test/server-v1.test.js`, which also captures the child's stderr + exit code so an unexplained dead server names its killer.

### 47. `/v1/*` is a FROZEN public contract — additive changes only, and the Host check is load-bearing

`lib/api-v1.js` (mounted `app.route('/v1', …)` in server.js) is Clauge's external API — the Clauge Widget, Mission Control, and third-party scripts consume it, and the wire format is aiusage-compatible by owner decision (camelCase envelope, `resets_at` in lines, None-dropping). Rules: (a) NEVER remove/rename existing fields, labels, or line types — add new lines/fields instead (consumers render blindly on `type` + `label`; that's the schema's payoff); a breaking change requires bumping `V1_API_VERSION`. (b) The loopback Host-check middleware is the PRIMARY and ONLY anti-DNS-rebinding control — `/v1` is deliberately NOT in `READ_ONLY_API_PATHS`, so for a rebound page the request is browser-same-origin and CORS never engages; do not remove the check, and do not add `/v1` to the CORS allowlist without re-running that threat analysis. (c) Do NOT reuse `SNAPSHOT_SCHEMA_VERSION` (landmine #37's cross-repo iOS contract) for `/v1` versioning. Spec: `docs/superpowers/specs/2026-07-16-v1-usage-local-api-design.md`.

### 48. Scoped quota windows: labels are DATA from the wire — never hardcode a bucket name again

claude.ai reports model/surface-scoped quota via a generic `limits[]` array (entries with a non-null `scope`); the flat `seven_day_sonnet`/`seven_day_opus` keys are permanently null (2026-07-18 pivot). `normalizeUsage` parses this into `scopedWindows: [{label, pct, resetsAt, isActive, group, source}]` — defensive by contract: finite pct clamped 0–100, control-chars stripped, labels capped 40 chars, max 8 entries, wire order (spec: `docs/superpowers/plans/2026-07-18-fable-scoped-limits-plan.md`). Consumers must tolerate the key being ABSENT (old stored records) and must data-gate zero entries. Hero `fiveHour`/`sevenDay` synthesize from `kind: session`/`weekly_all` when the flat keys vanish — flat keys win when present; this fallback is what keeps alerts/projection/usage-history alive on migrated accounts (`WINDOW_KEYS`/`WATCHED_WINDOWS` allowlists exclude the array itself automatically). Rendering rules: dashboard inserts labels only through `escapeHtml`, popover only via `textContent`, the widget sanitizes and renders scoped gauges BLUE always (owner decision — never the green/orange/red thresholds; cap 2 gauges, widget spec §4 rev 5/5.1). `/v1` emits additive `Weekly (<label>)`/`Session (<label>)` lines deduped against the legacy emitters (landmine #47 still governs). The iCloud snapshot carries `scopedWindows` verbatim inside `usage.plan` with `SNAPSHOT_SCHEMA_VERSION` still 1. If a surface ever shows a phantom empty bucket or a hardcoded model name creeps back in, this landmine was violated.

### 49. Edit/Write tool payloads DECODE `\u` escape sequences into raw bytes — never put escape-form control chars in tool-written content

Writing source or docs through JSON-carrying editor tools turns a literal `U+0000`-style sequence in the payload into the actual control BYTE, silently corrupting the file to binary (bit an Edit call on 2026-07-18 and a Write call + a subagent Edit on 2026-07-19 — three hits in two days). Safe constructions when a control-char range is needed: JS — build the RegExp via `String.fromCharCode` (see `lib/usage-store.js`); Swift — `CharacterSet.controlCharacters`; prose/docs — spell ranges as `U+0000-U+001F`. After ANY write that involves escape-bearing content, byte-scan the file: `python3 -c "b=open('<file>','rb').read(); print([hex(c) for c in b if (c<0x20 and c not in (9,10,13)) or c==0x7f])"` must print `[]`. Repairs go through python byte-replace, never another Edit. This entry's own first write fell into the trap: a raw NUL sat in THIS file until 2026-07-19, making grep classify AGENTS.md as binary and silently return EMPTY for every pattern — so agents grepping the governance file got nothing and no error. Symptom to recognize: grep finds nothing in a doc that Read clearly shows; response: byte-scan the doc, repair via python.

### 50. `clauge status --install` must persist Homebrew's stable `opt/` path — never the resolved keg

`process.execPath` resolves `/opt/homebrew/bin/clauge` through to the version-addressed keg (`Cellar/clauge-cli/<version>/…`), and `brew upgrade` DELETES old kegs — persisting the resolved path into `~/.claude/settings.json` killed the statusline on the next upgrade while the host kept showing a stale last render (field incident 2026-07-19, fixed in v1.3.8). `stabilizeBrewPath()` in `lib/cli/status-install.js` rewrites `<prefix>/Cellar/<formula>/<version>/` → `<prefix>/opt/<formula>/` for both default prefixes; a wiring-guard test in `test/cli/status-install.test.js` fails if the stabilization call is ever removed (mutation-verified). Do not bypass it, and when smoke-testing a brew release, run the EXACT command string stored in `settings.json` — not `clauge status` from PATH; the two can diverge, and the divergence is precisely the bug.

### 51. `Claude Code-credentials` trusts the MESSENGER (`/usr/bin/security`), not the visitor — the DMG build reads it through that tool, and signing does NOT fix the prompt

Claude Code reads and writes its keychain item by forking `/usr/bin/security` (binary strings of 2.1.276: `find-generic-password` ×10, `add-generic-password` ×8, `delete-generic-password` ×5, `set-generic-password-partition-list` ×0) and never sets a trusted-app or partition list, so the item's ACL partition list is exactly `("apple-tool:")`. An in-process read (the `security_framework` crate) is therefore a partition mismatch — securityd logs `ACL partition mismatch: client cdhash:… ACL ("apple-tool:")` — and raises the "Clauge wants to use your confidential information…" password dialog. "Always Allow" IS recorded, and is wiped again when Claude Code next rewrites the item (measured on the owner's Mac 2026-09-18: granted 13:25:09, gone by 15:42:02 BDT; two of three prompts landed within ~20 s of a Claude Code auto-update; all three fired mid-session against a Clauge process up 27 days — never "at login").

**Since v1.3.9 the DMG flavor reads via `read_via_security_cli()` in `src-tauri/src/keychain.rs`**: absolute path, fixed argv, no shell, `-w` only (never `-g`, which prints the password to STDERR — and stderr's first line is what lands in `KeychainError::SecurityCli`). The tool's exit code is the LOW BYTE of the OSStatus: 44 = not found (-25300, observed live), 51 = auth failed (-25293), 128 = user canceled (-128). Every credentials blob on every platform parses through `parse_creds_blob()`, which keeps only the JSON error's category + line/column — raw `serde_json` errors embed the offending VALUE (`invalid type: string "<token>"…`) and would carry a token into `log::warn!` and the webview. The in-process `read_from_keychain_services` is cfg-gated to the **MAS flavor only**; the wiring-guard test `dmg_reader_uses_security_cli_not_in_process_keychain` fails if the DMG path ever goes back, if that gate is loosened, or if `-g` appears (mutation-verified on both assertions). No fallback to the in-process read — a silent fallback resurrects the dialog.

**There is a SECOND, different prompt — do not confuse the two.** Clauge's OWN item `com.clauding.clauge.claude-ai-session` is still read in-process (`claude_ai_session.rs::read_stored_cookie`). Its ACL is a classic trusted-application list keyed to each build's code hash — securityd prints `displaying keychain prompt … ThresholdAclSubject(1 of 40)[CodeSignatureAclSubject[legacyHash:…][path:/Applications/Clauge.app…` (40 entries = 40 builds the owner has approved). The DMG is ad-hoc signed, so every release has a new cdhash and this prompt fires **once per app UPDATE** (observed 16 s after the first 1.3.9 launch, 23:19:29 BDT 2026-09-18); "Always Allow" then sticks until the next update, because nobody rewrites that item (cdat == mdat since 2026-05-14). Telling them apart in the log: the Claude Code item logs `ACL partition mismatch … ("apple-tool:")` + `asking user about XARA partition`; Clauge's own item logs the `ThresholdAclSubject` line with NO partition line. **This second prompt is the one Developer-ID signing WOULD fix** (a stable designated requirement instead of a per-build hash). Never report "the keychain prompt is fixed" without saying which one — a user who updates WILL see this box once.

**Things that do NOT fix the `Claude Code-credentials` prompt — each refuted with log evidence, do not re-propose them as "the keychain fix":** Developer-ID signing + notarization (changes the entry from `cdhash:` to `teamid:`, but it lives in the same list that gets wiped — still worth doing for Gatekeeper, update-safety, and as the cure for the per-update prompt above); adding Clauge by hand in Keychain Access or via `set-generic-password-partition-list` (lifetime: hours); "Allow all applications" (insecure); the MAS build (the App Sandbox cannot use the CLI path; it still reads in-process and still prompts).

**Known holes, deliberately left open (owner approved a narrow fix):** (a) the MAS flavor keeps prompting, and its wizard/README copy says so; (b) `Command::output()` has no timeout and `keychain_cache::get_or_load` holds its Mutex across the read, so a `security` child blocked on a login-keychain UNLOCK dialog stalls every later caller until the user answers — strictly rarer than the old per-read ACL dialog it replaces, but unbounded; (c) `keychain_cache.rs` swallows an expired-token re-read failure at `log::debug!`, below the shipped `info` filter; (d) `connections.rs` collapses every `KeychainError` into "Claude Code: not installed". If Anthropic fixes upstream (anthropics/claude-code #77697, #89985, #87348, #94491) by attaching a real trusted-app list, re-test this path.

**Verifying any change here needs a real surface, not unit tests:** run `/usr/bin/log stream --style ndjson --predicate 'subsystem == "com.apple.securityd" AND category == "integrity"'` and confirm zero `ACL partition mismatch` lines across a launch, a forced refresh, and a Claude Code token rewrite; the `#[ignore]` test `live_security_cli_read_is_ok` exercises the real read without printing the token. **Forensics traps:** zsh has a builtin `log` that shadows `/usr/bin/log` — piped into grep it fails silently and looks like clean negative evidence, so always use the absolute path; count with `--style ndjson` + jq (`--style compact` inflated a count from 3 to 5); never run `security dump-keychain`, `-w` or `-g` from an agent (dialog storm / secret exposure) — pipe a `-w` read into a key-names-only `jq` if you must probe; an investigation's own `security` calls contaminate same-day burst correlations.

### 52. A green release run is NOT a delivered release — three silent gaps bit v1.3.9 in one night

**(a) The `latest.json` upload race is not harmless.** Both matrix jobs in `release.yml` upload an asset named `latest.json` to the same GitHub Release (`includeUpdaterJson: true`); the in-file comment calls this "last-writer-wins… harmless". On v1.3.9 the Windows job's replace landed mid-flight against the macOS job's copy, GitHub answered `Not Found` on *update-a-release-asset*, the Windows job FAILED at "Build, sign, and publish artifacts" — AFTER the Release was already public with 5 of 6 assets — and `mirror-updater` + `dispatch-homebrew` were skipped. Nothing reached users (the gh-pages feed stayed on the old version), but the release page was half-finished. **Recovery: `gh run rerun <run-id> --failed`** — same tag, same commit, no retag; with the other platform job finished there is nothing to race. The rebuilt installer gets a NEW signature, so afterwards verify the feed's per-platform `signature` equals the uploaded `.sig` asset byte-for-byte (a stale signature silently breaks every update on that platform).

**(b) A green "Dispatch Homebrew tap bump" job proves nothing.** GitHub auto-disables a repo's scheduled workflows after 60 days without commits (`state: disabled_inactivity`), and the tap only gets commits on releases — so any gap over 60 days between releases kills `auto-update.yml`. A DISABLED workflow swallows `repository_dispatch`: the POST still returns 204, `curl -f` passes, the release job goes green, and the tap never moves. v1.3.9 shipped 61 days after v1.3.8; the tap's workflow had been disabled ~8 hours earlier. **Recovery:** `gh workflow enable auto-update.yml --repo clauding-lab/homebrew-tap`, then the manual fallback `gh workflow run auto-update.yml --repo clauding-lab/homebrew-tap -f version=X.Y.Z`. Check the state BEFORE tagging: `gh api repos/clauding-lab/homebrew-tap/actions/workflows/auto-update.yml -q .state` must print `active`.

**(c) PR CI never compiles Rust for Windows.** `check.yml`'s `js-tests-windows` job runs `npm test` only; Windows Rust is first compiled by `release.yml` at TAG time (`cargo test --locked`). Any Rust change touching a Windows path (`#[cfg(target_os = "windows")]`, shared helpers) is unverified until the release run — read the Windows call sites and Windows-only tests by hand before tagging, and expect the tag run to be the real gate.

**Release-done checklist (all must be observed, not inferred from CI):** run conclusion `success` on BOTH platform jobs · 6 assets on the Release · gh-pages `latest.json` = new version with a signature per platform, each equal to its uploaded `.sig` · every feed URL answers 200 · the CDN copy (`clauding-lab.github.io/clauge/latest.json`, lags a few minutes) = new version · tap cask AND formula = new version with `sha256` equal to the Release assets' `digest` · the owner's installed app actually on the new version (the app is a DMG install that self-updates at launch via `check_for_updates`; only `clauge-cli` is on Homebrew). Durable cures for (a) and (b) are `release.yml` edits — VISION.md "Needs Sign-Off"; propose, don't apply.
